...
fn warm_solve_on_matching_pennies_is_the_exact_fixpoint() {
    ...
    let node = tree.node_mut(0);
    solve_node(node, 16, false, false);
...
fn warm_solves_are_bitwise_deterministic() {
    ...
    for _ in 0..3 {
        solve_node(first.node_mut(0), 16, false, false);
        solve_node(second.node_mut(0), 16, false, false);
    }
...
fn average_policy_divides_by_solves_or_falls_back() {
    assert_eq!(
        average_policy(&[8.0, 24.0], 32.0, &[0.0, 0.0]),
        [0.25, 0.75]
    );
    assert_eq!(average_policy(&[8.0, 24.0], 0.0, &[0.1, 0.9]), [0.1, 0.9]);
}
