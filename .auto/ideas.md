...
- A future destination-writing State transition API could construct expansion states directly in `MaybeUninit<Node>` without first cloning the parent into the final slot. The current capability-gated in-place path removes the large returned-state copy for bundled games, but a sound public destination API needs a design that cannot let third-party implementations leave memory uninitialized.
