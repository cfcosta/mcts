...
- **Kept:** removed cached legal-action `Vec`s from both bundled states. TTT is now a 6-byte `Copy` state and UTTT a 44-byte `Copy` state; UTTT stores only next-board/occupied metadata. Legal vectors are generated on demand with exact capacities derived from empty masks/occupancy rather than retained or worst-case buffers.
...
- **Kept:** selectively amortized inverse-sqrt and sqrt-log lookup tables remove division/sqrt/log from UCB scans. Immutable power-of-two tiers are built after a branching expansion and shared process-wide, retaining branch-free static slices while amortizing setup across repeated Mcts instances and full games. Combined with the one-pass scan, wide-512 is ~88% faster than baseline.
...
- The current six-core-workload primary best is `geomean_ns=858955.150` on the nine-round CPU-6 harness, 27.8% below the core-search baseline. Before the metric reset, the eight-workload score was 71.3% below baseline. Latest Criterion cross-check after direct construction: 10k TTT searches are 80-83% faster and 1k UTTT searches 84-85% faster versus untouched `initial`; shared tiers subsequently improved full-game TTT another 1.9% (0.363 ms), while UTTT remained around 9.22 ms.
