...
- **Kept:** a concrete rejection-corrected Lemire sampler replaced large generic `gen_range` monomorphizations. A shared 4.5 KiB compile-time table maps `(9-bit empty mask, uniform rank)` directly to an empty cell for both games.
...
- The current six-core-workload primary best is `geomean_ns=998719.735` on the nine-round CPU-6 harness. Before the metric reset, the eight-workload score was 71.3% below baseline. Latest full Criterion cross-check: every search budget/position is 66-82% faster; full-game TTT is 68% faster and full-game UTTT 77% faster versus untouched `initial`.
